...
void onMsgReceived() {
    ...
    if((rxMsg.id == CAN_ID)){
        ...
        if(((rxMsg.data[0]==0xFF) & (rxMsg.data[1]==0xFF) & (rxMsg.data[2]==0xFF) & (rxMsg.data[3]==0xFF) & (rxMsg.data[4]==0xFF) & (rxMsg.data[5]==0xFF) & (rxMsg.data[6]==0xFF) & (rxMsg.data[7]==0xFC))){
            ...
            // Get motor status independent to the modes.
            }
...
void enter_setup_state(void){
    ...
    wait_us(10);
    printf(" %-4s %-31s %-5s %-6s %.1f\n\r", "h", "Temp Cutoff (C) (0 = none)", "0", "80", TEMP_CUTOFF);
    wait_us(10);
            ...
            if(c == 13){
                switch (cmd_id){
                    ...
                    case 't':
                        ...
                        break;
                    case 'h':
                        TEMP_CUTOFF = fmaxf(fminf(atof(cmd_val), 80.0f), 0.0f);
                        break;
                    case 'c':
...
int main() {
    ...
    if(isnan(TEMP_CUTOFF) || TEMP_CUTOFF==-1){TEMP_CUTOFF = 60.0f;}
